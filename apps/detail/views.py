...
def index(request):
    ...
    groups = machines.all_machines(GroupInfo)
    ...
    context = {'title': "首页", 'platform': '1', 'cabinets': cabinets, 'networks': networks, 'groups': groups, 'othermachines': othermachines, 'phy_classify': phy_classify, 'vir_classify': vir_classify}
...
def machine_list(request):
    ...
    if types == 'cb':
        ...
        cname = request.GET.get("phyname")
    elif types == 'gp':
        paginator = Paginator(machines.all_machines(GroupInfo), 10)
        template = "detail/list_grp.html"
    elif types[0:3] == 'gct':
        paginator = Paginator(machines.filter_con_servers(Conn_Grp_Id=int(types[3:])), 10)
        template = "detail/grp_contains.html"
        cname = request.GET.get("grpname")
    page = PageDeal().pages_deal(int(request.GET.get('p')), paginator)
    if cname:
        context = {'title': "设备列表", 'page': page, 'types': types, 'sn_states': sn_states, 'cname': cname}
